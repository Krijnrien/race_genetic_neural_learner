...
public class GeneticAlgorithm {
	// Maximum order used in weight calculation
	private static final float MAX_PERMUTATION = 0.3f;
	// Maximum mutation rate when applying random sigmoid weight
	private static final float MUTATION_RATE = 0.15f;
	// Keeping track of amount of genomes in population
	private int currentGenome;
	// Number of total population size
	private int totalPopulation;
	// ID to keep track of current driving genome
	private int genomeID;
	// ID to keep track of current generation. aka evolved population
	...
	private ArrayList<Genome> crossOver(Genome g1, Genome g2) {
		...
		genomeID++;
		// Go from start to crossover point, copying the weights from genome 1 to children.
		for (int i = 0; i < crossover; i++) {
			genome1.weights.add(g1.weights.get(i));
			genome2.weights.add(g2.weights.get(i));
		}
		// Go from start to crossover point, copying the weights from genome 2 to children.
	...
	private void mutate(Genome genome) {
		for (int i = 0; i < genome.weights.size(); ++i) {
			...
			if (randomSigmoid < MUTATION_RATE) {
				genome.weights.set(i, genome.weights.get(i) + (randomSigmoid * MAX_PERMUTATION));
			}
		}
	}
